...
window.Bookmarker = {
	...
	loadScript: function(url, testFn, callback) {
		...
		function _loadDirectly() {
			var scr = document.createElement('script');
			scr.type = 'text/javascript';
			scr.src = url;
			scr.onload = scr.onreadystatechange = _loaded;
	...
	loadDependencies: function(callback) {
		...
		this.loadScript(jQueryLib, jQueryTest, setjQuery);
	},
	...
	load: function(url, space) {
		...
		this.urlBase = anchor.protocol + '//' + anchor.host;
		...
		this.loadDependencies(function() {
			...
			self.iframe.addEventListener('load', function() {
				var message = self.getMessage(space, { local: false });
